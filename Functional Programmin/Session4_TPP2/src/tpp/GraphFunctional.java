...
public class GraphFunctional {
    ...
    private static void exercise1_NodeDegrees(List<List<Integer>> graph) {
    	...
    	List <Integer> degrees = graph.stream().map(List::size).toList();
    	//graph.stream.map(x->x.size()).toList();
    	
    	System.out.println("Grados de los nodos: ");

        for (int i = 0; i < degrees.size(); i++) {
            System.out.println("Nodo " + i + ": " + degrees.get(i));
        }
    }
    ...
    private static void exercise2_NeighborsOfNeighbors(List<List<Integer>> graph, int start) {
    	...
    	List <Integer> startingNodeNeighbors = graph.get(start);
    	
    	List<Integer> neighborsOfNeighbors = startingNodeNeighbors.stream().flatMap(node->graph.get(node).stream()).toList();
    	
    	System.out.println("Neighbors of Starting node (" + start + "):" + startingNodeNeighbors );
    	
    	System.out.println("Neighbors of neighbors of node (" + start + "): " + neighborsOfNeighbors );
    ...
    private static void exercise3_SecondLevelNeighborsUnique(List<List<Integer>> graph, int start) {
    	...
    	List<Integer> neighborsOfNeighbors = startingNodeNeighbors.stream()
    			.flatMap(node->graph.get(node).stream())
    			.filter(x->x!=start) //starting node
    			.filter(x ->!startingNodeNeighbors.contains(x))//neighbors of stating node
    			.distinct()
    			.sorted()
    			.toList();
    	
    	System.out.println("Neighbors of neighbors no duplicates and excluding start of node (" + start + "): " + neighborsOfNeighbors );
    }
    ...
    private static void exercise4_CountEdges(List<List<Integer>> graph) {
        // TODO: implement this method according to the exercise description.
    	int degrees= graph.stream().map(x->x.size()).reduce(0,(x,y)-> x+y);
    	//.reduce(0,Integer::sum)
    	int edges = degrees/2;
    	
    	System.out.println("Number of degrees: " + degrees );
    ...
    private static void exercise5_MaxDegreeNodes(List<List<Integer>> graph) {
    	...
    	List <Integer> degrees = graph.stream().map(x->x.size()).toList();
    	int maxDegree = degrees.stream().max(Integer::compareTo).orElse(0);
    	
    	List<Integer> maxDegreeNodes = IntStream.range(0, degrees.size())
    			.filter(i -> graph.get(i).size() == maxDegree) 
    			.boxed().toList();
    			
    			//IntStream as long as the size of the graph from 0 to 4, bc we want to filter
        System.out.println("Nodes with max degree (" + maxDegree + "): " + maxDegreeNodes);
    ...
    public static void exercise1_FilterReusablePredicates(List<List<Integer>> graph) {
        // TODO: implement this method according to the exercise description.
        List<Integer> numbers = List.of(1,2,3,4,-1,-2,-3);
        Predicate<Integer> isEven = x -> x % 2 == 0; 
        Predicate<Integer> isPositive = x -> x > 0;
    	
        Predicate<Integer> isEvenAndPositive = isEven.and(isPositive);
        List<Integer> filteredNumbers = numbers.stream().filter(isEvenAndPositive).toList();
        
        System.out.println("Positive and even numbers: " + filteredNumbers);
    ...
    public static void exercise2_GraphPredicates(List<List<Integer>> graph) {
        // TODO: implement this method according to the exercise description.
    	BiPredicate<Integer, Integer> isNeighbour=(node, k) -> graph.get(node).contains(k);
    ...
    public static void exercise3_FunctionReusable(List<List<Integer>> graph) {
        ...
        Function<Integer, Integer> doublePlusOne = doubleValue.andThen(increment);
        List<Integer> nums = List.of(1, 2, 3, 4).stream().map(doublePlusOne).toList();
    ...
    public static void exercise4_ConsumerReusable(List<List<Integer>> graph) {
        // TODO: implement this method according to the exercise description.
    	Consumer<Integer> print = x -> System.out.println("Node: " + x);
    	Consumer<Integer> debug = x -> System.out.println("[debug] visiting: " + x);
        Consumer<Integer> printSquared = x -> System.out.println("Node squared: " + (x * x));
        
        visitNodes(List.of(1, 2, 3), print);  
        visitNodes(List.of(1, 2, 3), debug);  
        visitNodes(List.of(1, 2, 3), printSquared);

        print.andThen(debug).andThen(printSquared).accept(4); 
    ...
    public static void exercise5_PathValidationReusable(List<List<Integer>> graph) {
        // TODO: implement this method according to the exercise description.
    	Predicate<Integer> positiveIndex = index -> index >= 0;  
        Predicate<Integer> lessThanSize = index -> index < graph.size();
    	
        Predicate<Integer> validIndex=positiveIndex.and(lessThanSize);
        
        List<Integer> validPath = List.of(0, 2, 3); 
        List<Integer> invalidPath = List.of(0, 5, 3); //5 esta fueras
        
        System.out.println("Valid: " + validatePath(validPath, validIndex));
        System.out.println("InValid: " + validatePath(invalidPath, validIndex));
