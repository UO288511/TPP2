...
public class FunctionalStream {
    ...
    public static void ejercicio1PredicateStringIsEmpty() {
        ...
        // TODO: implement Predicate<String> using String::is	Empty
        // and its equivalent lambda, testing with empty and non-empty strings.
        
        Predicate<String> isEmptyRef = String::isEmpty;
        Predicate<String> isEmptyLambda = x -> x.isEmpty();
        
        System.out.println(isEmptyRef.test(""));
        System.out.println(isEmptyLambda.test(""));
        System.out.println(isEmptyRef.test("gggg"));
        System.out.println(isEmptyLambda.test("ffff"));
    ...
    public static void ejercicio2PredicateCharacterIsUpperCase() {
        ...
        System.out.println(isUpperCaseRef.test('A'));  
        System.out.println(isUpperCaseLambda.test('A'));
        System.out.println(isUpperCaseRef.test('l')); 
        System.out.println(isUpperCaseLambda.test('l'));
    ...
    public static void ejercicio3FunctionStringToLength() {
        ...
        System.out.println(lengthRef.apply("asdf")); 
        System.out.println(lengthLambda.apply("asdf")); 
    ...
    public static void ejercicio4FunctionStringToIntegerParse() {
        ...
        System.out.println(parseIntRef.apply("100").getClass()); 
        System.out.println(parseIntLambda.apply("100").getClass()); 
    ...
    public static void ejercicio5ConsumerPrintWithSystemOut() {
        ...
        printRef.accept("hola");  
        printLambda.accept("dfgfdg");
    ...
    public static void ejercicio6ConsumerAddToList() {
        ...
        Consumer <String> addToListRef= list::add;
        Consumer<String> addToListLambda = s -> list.add(s);
        
        addToListRef.accept("One");
        addToListLambda.accept("Two");
    ...
    public static void ejercicio7SupplierRandomNumber() {
        ...
        System.out.println(randomRef.get());  
        System.out.println(randomLambda.get());
    ...
    public static void ejercicio8SupplierCreateStringBuilder() {
        ...
        Supplier<StringBuilder> stringBuilderRef = StringBuilder::new;
        Supplier<StringBuilder> stringBuilderLambda = () -> new StringBuilder();
        
        StringBuilder sb1 = stringBuilderRef.get();
        StringBuilder sb2 = stringBuilderLambda.get();
    ...
    public static void runBlock2StreamsBasics() {
        ...
        System.out.println("Count: " + numbers2.stream().count());
        // - Use anyMatch() to check if there is any even number.
        System.out.println("any even?: " + numbers2.stream().anyMatch(n -> n % 2 == 0));
        // - Use allMatch() to check if all are positive.
        System.out.println("all positive?: " + numbers2.stream().allMatch(n -> n > 0));
        ...
        System.out.println("none even?: " + numbers3.stream().noneMatch(n -> n % 2 == 0));
        ...
        List<String> words4 = Arrays.asList("java", "python", "c", "go", "haskell");
        // - Create a stream, filter those with length > 3 and show them with forEach.
        System.out.println("length >3?: ");
        words4.stream().filter(x -> x.length() > 3)
        ...
        List<String> cities5 = Arrays.asList("Gijon", "Oviedo", "Mieres", "Aviles", "Madrid");
        // - Filter based on a condition and sort alphabetically before showing.
        System.out.println("start with A or G and sorted:");
        cities5.stream().filter(x -> x.startsWith("A") || x.startsWith("G"))
        				.sorted()
        				.forEach(System.out::println);
        System.out.println();
        ...
        List<Integer> numbers6 = Arrays.asList(1, 2, 2, 3, 4, 4, 5);
        // - Use distinct() to remove duplicates.
        // - Filter only even ones and count how many there are.
        System.out.println("distinct even numbers count: ");
        System.out.println(numbers6.stream().distinct().
        									filter(n -> n % 2 == 0).
        									count());
        System.out.println();
        ...
        // TODO:
        // - Define a list of integers.
        List<Integer> numbers7 = Arrays.asList(10, 3, 7, 20, 25, 2, 30);
        // - Filter greater than a certain value.
        // - Use limit() to keep the first results and show them.
        System.out.println("first 2 numbers greater than 10: ");
        numbers7.stream().filter(x -> x>10).
        					limit(2).
        					forEach(System.out::println);
    ...
    private static void runOption1ShowOriginalText() {
        ...
        TEXT_LINES.stream().forEach(System.out::println);
    ...
    private static void runOption2BasicStatistics(List<String> words) {
        ...
        // - Calculate total words with count().
    	 System.out.println("Total words: " + words.stream().count());
        // - Calculate distinct words with distinct().count().
    	 System.out.println("Distinct words: " + words.stream().distinct().count());
        // - Calculate how many have length > 5 with filter().count().
    	 System.out.println("words length > 5: " + words.stream().filter(x -> x.length() > 5 ).count()); 
    ...
    private static void runOption5CheckWordConditions(List<String> words) {
    	...
    	System.out.println("words lenght > 10?: " + words.stream().anyMatch(x -> x.length() > 10));
        // - Use allMatch to check if all words have length > 1.
    	System.out.println(" all words lenght > 1?: " + words.stream().allMatch(x -> x.length() > 1));
        // - Use noneMatch together with containsDigit to check that no word has digits.
    	System.out.println("words no digits?: " + words.stream().noneMatch(x -> containsDigit(x)));
    ...
    private static void runOption6FirstWordsWithMinLength(Scanner sc, List<String> words) {
        ...
        words.stream().filter(x -> x.length() >= n)
        .limit(5)
        .forEach(System.out::println);
    }
}
